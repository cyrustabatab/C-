...
def hierholzers(g,is_eulerian):

    is_graph_eulerian,start = is_eulerian(g)

    if not is_graph_eulerian:
        print("Graph has no Eulerian Cycle or Eulerian Path")
    else:
        if not start:
...
def is_directed_eulerian(g):
    ...
    for node in g:
        for neighbor in node.get_neighbors():
            in_degrees[neighbor.id] += node.get_edge_count(node)
...
def is_undirected_eulerian(g):
    ...
    number_nonzero = sum(node.num_neighbors > 0 for node in g)
